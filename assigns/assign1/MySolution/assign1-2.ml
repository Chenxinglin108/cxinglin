...
let string_merge cs1 cs2 =
  let rec merge s1 s2 acc =
    ...
    | s1,s2->
      ...
      if int_of_char s1.[0] <= int_of_char s2.[0]then
       String.make 1 s1.[0]  ^ merge (String.sub s1 1 (String.length s1 - 1)) s2 acc
      else
        String.make 1 s2.[0] ^ merge s1 (String.sub s2 1 (String.length s2 - 1)) acc
  in
  merge cs1 cs2 "" ;;
